...
class Auth_lite extends CI_Controller {
    ...
    function lite_library()
	{
		...
		$this->data['user_full_name'] = (! empty($user)) ? $user['first_name'].' '.$user['last_name'] : null;
		$this->data['user_phone'] =(! empty($user)) ?  $user['phone'] : null;
		$this->data['user_last_login'] = (! empty($user)) ? date('jS M Y @ H:i:s', strtotime($user['date_last_login'])) : null;
		$this->data['user_group_desc'] = (! empty($user)) ? $user['desc'] : null;
