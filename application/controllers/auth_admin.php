...
class Auth_admin extends CI_Controller {
	...
	*/
    ...
    function manage_user_accounts()
    {
		...
		else if ($this->input->post('update_users') && $this->flexi_auth->is_privileged('Manage Users')) 
		{
	...
	function update_user_account($user_id)
	{
		...
		$sql_where = array($this->flexi_auth->db_table('user_acc').'.'.$this->flexi_auth->db_column('user_acc', 'id') => $user_id);
    ...
    function manage_user_groups()
    {
		...
		if ($this->input->post('delete_group') && $this->flexi_auth->is_privileged('Manage Users')) 
		{
			$this->load->model('demo_auth_admin_model');
    ...
    function manage_privileges()
    {
		...
		if ($this->input->post('delete_privilege') && $this->flexi_auth->in_group('Developer')) 
		{
			$this->load->model('demo_auth_admin_model');
	...
	function update_privilege($privilege_id)
	{
		...
		if (! $this->flexi_auth->in_group('Developer','Admin'))
		{
	...
	function list_user_status($status = FALSE)
	{
		...
		if (! $this->flexi_auth->in_group('Developer'))
		{
			...
			$sql_where[$this->flexi_auth->db_column('user_group', 'name').' !='] = 'Developer';
		}
	...
	function delete_unactivated_users()
	{
		...
		if ($this->input->post('delete_unactivated') && $this->flexi_auth->is_privileged('Manage Users'))
		{
			$this->load->model('demo_auth_admin_model');
