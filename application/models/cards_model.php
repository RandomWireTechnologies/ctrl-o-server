...
class Cards_model extends CI_Model {
	...
	function get_user_cards($user_id) {
		...
		$query = $this->db->get_where("cards",array('user_id' => $user_id));
		return $query->result_array();
	}

	function get_user_logs($user_id) {
		$this->db->select("*,nodes.name as node_name,cards.name as card_name,cards.id as card_id");
		$this->db->from("log");
		$this->db->join("nodes","nodes.id = log.node_id");
		$this->db->join("cards","cards.id = log.card_id");
