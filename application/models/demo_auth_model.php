...
class Demo_auth_model extends CI_Model {
	...
	function register_account()
	{
		...
		if ($this->form_validation->run())
		{
			...
			if ($response)
			{
				...
				$email_data = array(
				    'user_id' => $response,
				    'name' => $this->input->post('register_first_name')." ".$this->input->post('register_last_name'),
				    'username' => $username,
				    'email' => $email
				);
                                $email_notify = $this->config->item('new_member_notify_email');
                                if ($email_notify != "") {
			            $this->flexi_auth->send_email($email_notify, 'New Member Registered', 'registration_report.tpl.php', $email_data);
                                }
				###+++++++++++++++++###
	...
	function update_account()
	{
		...
		if ($this->form_validation->run())
		{
			...
			// Redirect user.
			($response) ? redirect('user/profile') : redirect('auth_public/profile');
		}
		else
		{		
			// Set validation errors.
