...
class Demo_auth_admin_model extends CI_Model {
	...
	function get_user_accounts()
	{
		...
		$sql_select = array(
			$this->flexi_auth->db_table('user_acc').'.'.$this->flexi_auth->db_column('user_acc', 'id'),
			$this->flexi_auth->db_table('user_acc').'.'.$this->flexi_auth->db_column('user_acc', 'email'),
			$this->flexi_auth->db_table('user_acc').'.'.$this->flexi_auth->db_column('user_acc', 'suspend'),
			$this->flexi_auth->db_table('user_group').'.'.$this->flexi_auth->db_column('user_group', 'name'),
			'first_name',
			'last_name',
		);
		...
		{
			$sql_where[$this->flexi_auth->db_table('user_group').'.'.$this->flexi_auth->db_column('user_group', 'name').' !='] = 'Developer';
	...
	function update_user_accounts()
    {
		...
		if ($this->flexi_auth->is_privileged('Manage Users')) 
		{
	...
	function update_user_account($user_id)
	{
		...
		if ($this->form_validation->run())
		{
			...
			// In this example, the primary key column and value is 'id' => $user_id.
			$profile_data = array(
				'user_id' => $user_id,
				...
				'company' => $this->input->post('update_company'),
				'address' => $this->input->post('update_address'),
				'address2' => $this->input->post('update_address2'),
				'city' => $this->input->post('update_city'),
				'state' => $this->input->post('update_state'),
				'zipcode' => $this->input->post('update_zipcode'),
