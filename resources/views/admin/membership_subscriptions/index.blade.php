...
<div class="main-content">
    ...
    <div class="modal fade modal-scalled" id="form-modal" aria-hidden="true" role="dialog">
        <div class="modal-dialog open-modal-center">
            {!! Form::open([
                'route' => 'admin.membership-subscriptions.store', 
                'id'    => 'add-form', 
                ...
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Add Membership Subscription</h4>
                </div>
                
                <div class="modal-body">
                    <div class="row">
                        @include('admin.membership_subscriptions.form')
                    </div>
                </div>
...
<script>
    Grid.setOptions({
        deleteConfirm: 'Do you really want to delete this Membership Subscription?',
        loadRoute: "{{ route('api.membership-subscriptions.list') }}",
        updateRoute: "{{ route('admin.membership-subscriptions.update', ['id' => ':id']) }}",
        insertRoute: "{{ route('admin.membership-subscriptions.store') }}",
        deleteRoute: "{{ route('admin.membership-subscriptions.destroy', ['id' => ':id']) }}",
        fields: {!! $fields !!},
        rules: {!! $rules !!},
        messages: {!! $messages !!}
    }).render();
