...
<script>
    Grid.setOptions({
        ...
        loadRoute: "{{ route('api.nodes.list') }}",
        updateRoute: "{{ route('admin.nodes.update', ['id' => ':id']) }}",
        insertRoute: "{{ route('admin.nodes.store') }}",
        deleteRoute: "{{ route('admin.nodes.destroy', ['id' => ':id']) }}",
        fields: {!! $fields !!},
        rules: {!! $rules !!},
        messages: {!! $messages !!}
    }).render();
