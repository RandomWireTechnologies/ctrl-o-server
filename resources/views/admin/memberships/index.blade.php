...
<script>
    Grid.setOptions({
        deleteConfirm: 'Do you really want to delete this Membership?',
        loadRoute: "{{ route('api.memberships.list') }}",
        updateRoute: "{{ route('admin.memberships.update', ['id' => ':id']) }}",
        insertRoute: "{{ route('admin.memberships.store') }}",
        deleteRoute: "{{ route('admin.memberships.destroy', ['id' => ':id']) }}",
        fields: {!! $fields !!},
        rules: {!! $rules !!},
        messages: {!! $messages !!}
    }).render();
