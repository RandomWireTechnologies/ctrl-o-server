...
<script>
    Grid.setOptions({
        deleteConfirm: 'Do you really want to delete this Schedule?',
        loadRoute: "{{ route('api.schedules.list') }}",
        updateRoute: "{{ route('admin.schedules.update', ['id' => ':id']) }}",
        insertRoute: "{{ route('admin.schedules.store') }}",
        deleteRoute: "{{ route('admin.schedules.destroy', ['id' => ':id']) }}",
        fields: {!! $fields !!},
        rules: {!! $rules !!},
        messages: {!! $messages !!}
    }).render();
